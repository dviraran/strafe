    ...
    def forward(self, query, key, value, mask=None, dropout=None, backwards_only=False):
        ...
        if mask is not None:
            ...
            if not backwards_only:
                mask_shaped = torch.einsum('bi,bj->bij', (mask, mask)).unsqueeze(1).expand(scores.shape)
            else:
                mask_shaped = torch.triu(torch.einsum('bi,bj->bij', (mask, mask))).unsqueeze(1).expand(scores.shape)
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, hidden_size=300, num_attention_heads=4, dropout=0.3, backwards_only=False):
        ...
        self.linear_layers = nn.ModuleList([nn.Linear(hidden_size, hidden_size, bias=False) for _ in range(3)])
    ...
    def forward(self, query, key, value, mask=None, backwards_only=False):
        ...
        x, attn = self.attention(query, key, value, mask=mask, dropout=self.dropout, backwards_only=self.backwards_only)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        x = self.input_sublayer(x, lambda y: self.attention.forward(y, y, y, mask=mask))
...
class RNNBlock(nn.Module):
    ...
    def __init__(self, hidden, dropout=0.3, tr_dropout=0.3):
        super().__init__()
        self.rnn = torch.nn.GRU(input_size=hidden,hidden_size=hidden,batch_first=True)
