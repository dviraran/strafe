...
func = FunctionTransformer(func=tr_func,accept_sparse=True,validate=True,kw_args={'kwarg': 1})

# Changed
def gen_lr_pipeline(C=0.01):
    lr = LogisticRegression(
        class_weight='balanced', C=C, penalty='l1', fit_intercept=True, solver='liblinear', random_state=0, verbose=0,
        max_iter=200, tol=1e-1)
    ...
    clf_lr = Pipeline([('func', func), ('lr', lr)])
    clf_lr = lr
    return clf_lr
    # all_preds
