...
class Database(object):
    def __init__(self, config_path, schema_name, connect_args, cdm_schema_name, echo=False):
        print(config_path.format(**connect_args))
        self.engine = sqlalchemy.create_engine(config_path.format(**connect_args))
        self.meta = sqlalchemy.schema.MetaData(bind=self.engine, )
    ...
    def build_table_from_sql_file(self, sql_path, table_name, params, schema_name, replace=False):

        if replace or table_name not in self.get_all_tables(schema=schema_name).values:
            t = time.time()
            if replace:
    ...
    def fast_query(self, sql):
        ...
        """
        copy_sql = """copy ({query}) to stdout with csv {head} """.format(query=sql, head="HEADER")
    ...
    def get_all_tables(self, schema=config.OMOP_CDM_SCHEMA):
        """
        ...
        """
        sql = """select table_name from information_schema.tables {}; """.format(
            '' if schema is None else "where table_schema = '{}'".format(schema))
