...
class Cohort(object): 
    ...
    def build(self, db, replace=False):  # noqa
        ...
        if replace or self._cohort_table_name not in db.get_all_tables(schema=self._schema_name).values:
                t = time.time()
                if replace:
                    print('Regenerating Table (replace=True)')
                else:
                    print('Table not found in schema {}, regenerating'.format(self._schema_name))
        ...
        if not self._built:
            ...
            col_names = ['example_id','person_id','start_date','end_date'] + [self._outcome_col] + ['t'] +['e'] + ['year_of_birth'] + ['gender'] # Added age and gender to cohort features
